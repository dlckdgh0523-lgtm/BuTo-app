# AGENTS.md — BUTO Working Guide
## Codex implementation guide based on PRD.md

- Project: BUTO
- Base document: `BUTO_PRD_EN_v1.1.md`
- Purpose: This file defines how Codex should reason, challenge assumptions, and produce implementation artifacts for the BUTO Apps in Toss mini-app.
...
Codex must always align outputs with the following constraints.
...
Codex must challenge the following areas aggressively if they appear weak or underspecified:
...
When Codex is asked to design or implement something substantial, prefer this structure:
...
## 14. What Codex should produce well

Codex should be especially strong at producing:
...
A skill is a set of local instructions to follow that is stored in a `SKILL.md` file. Use the following skills when the task matches their scope.

### Available skills

- `docs-search`: Search and retrieve Toss / Apps-in-Toss documentation using the ax CLI via run-ax.sh. Use when a task depends on current Toss platform docs, TDS references, or release guidance. (file: `/Users/apple/.codex/skills/docs-search/SKILL.md`)
- `project-validator`: Validate Apps-in-Toss project configuration and structure. Use when checking required files, framework setup, package dependencies, or release-readiness basics. (file: `/Users/apple/.codex/skills/project-validator/SKILL.md`)
- `buto-toss-miniapp`: Implement, review, and refactor BUTO Apps-in-Toss flows with Toss-specific constraints and BUTO safety rules. Use for mini-app, API, contracts, policy, and release-feasibility work tied to Toss. (file: `/Users/apple/Desktop/buto/skills/buto-toss-miniapp/SKILL.md`)
- `buto-design-system`: Design and review BUTO mini-app and admin UI with safety-first visual rules, Korean copy guidance, and misuse-prevention checks. Use for screens, layouts, UI copy, state handling, and accessibility review. (file: `/Users/apple/Desktop/buto/skills/buto-design-system/SKILL.md`)

### How to use skills

- If the user names a skill explicitly, open that `SKILL.md` first and follow it.
- If the task clearly matches one of the skills above, use the minimal set of relevant skills without waiting for explicit naming.
- For Toss platform behavior that could be stale or undocumented, use `docs-search` before making claims.
- For Apps-in-Toss setup or config questions, use `project-validator` before proposing structural fixes.
- For BUTO product implementation, prefer `buto-toss-miniapp` for domain logic and `buto-design-system` for UX or UI decisions.
