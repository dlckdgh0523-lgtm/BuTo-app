...
const bundleLabel = readArgValue("--bundle-label") ?? `${environmentLabel}-${new Date().toISOString().slice(0, 10)}`;
...
console.log(`Wrote submission bundle to ${result.outputDir}`);

if (failOnBlockers && readiness.success.blockers > 0) {
  process.exit(1);
}

if (failOnWarnings && readiness.success.warnings > 0) {
  process.exit(1);
}
