...
import { fail, ok, type ApiResponse, type AuthenticatedUserSummary, type FaceAuthSession } from "../../../../packages/contracts/src/index.ts";
...
export class AuthService {
  ...
  loginCallback(): ApiResponse<{
    ...
    const user = this.getMe("client-1");
  ...
  createFaceAuthSession(userId: string, intent: FaceAuthSession["intent"], jobDraftId?: string) {
    ...
    const session = {
      ...
      jobDraftId,
      intent,
      provider: "TOSS_FACE_AUTH" as const,
  ...
  completeFaceAuth(userId: string, faceAuthSessionId: string, providerTransactionId: string, result: "SUCCESS" | "FAIL" | "CANCELLED") {
    ...
    if (!session || session.userId !== userId) {
      return fail("FACE_AUTH_SESSION_NOT_FOUND", "얼굴 인증 세션이 유효하지 않아요.");
    }
  ...
  assertValidFaceAuth(userId: string, faceAuthSessionId: string) {
    const session = this.store.faceAuthSessions.get(faceAuthSessionId);
    if (!session || session.userId !== userId) {
      return fail("FACE_AUTH_REQUIRED", "토스 얼굴 인증이 필요해요.");
    }

    if (!("verifiedAt" in session)) {
      return fail("FACE_AUTH_NOT_COMPLETED", "얼굴 인증 완료가 필요해요.");
    }

    if (Date.parse(session.expiresAt) <= Date.now()) {
      return fail("FACE_AUTH_EXPIRED", "얼굴 인증이 만료되었어요.");
    }
